...
<table class="cartTable table mt-5 table-borderless bg-dark1"v-if="cartData.length>0">
<thead class="bg-dark">
...
<tbody>
<tr v-for="item in cartData"style="border-bottom:2px solid #dcd9cb":key="item.id">

<td class="text-center pt-4">{{item.title}}</td>
<td class="text-center"><img class="cartImg":src="`${item.imageUrl}`" alt="" ></td>
...
<tfoot >
<tr>
<td colspan="5" v-if="totoaPrice >=1000" class="text-right text-white">(滿1000免運)</td>
</tr>
...
<td v-if="totoaPrice >=1000" class="h3 text-right">${{totoaPrice}}元</td>
...
<a class="h3 cartin font-weight-bold"href="#/product"><i class="fas fa-arrow-circle-left"></i>繼續購物</a>
<a class="h3 cartin font-weight-bold"href="#"@click.prevent="goCheckout"><i class="fas fa-arrow-circle-right"></i>結帳</a>
</div>
</div>
